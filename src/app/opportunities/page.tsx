                    ...
                    {topOpen.map(d=>(
                      <tr key={d.id} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="py-2 pr-3 text-xs text-slate-600 max-w-[130px] truncate">
                          <button onClick={()=>setSelectedDeal(d)} className="hover:text-blue-600 hover:underline text-left truncate w-full">{d.title}</button>
                        </td>
                      ...
                      return (
                        <tr key={d.id} className="hover:bg-emerald-50/30 transition-colors">
                          ...
                          <td className="py-2 pr-3 text-xs text-slate-600 max-w-[150px] truncate">
                            <button onClick={()=>setSelectedDeal(d)} className="hover:text-emerald-600 hover:underline text-left truncate w-full">{d.title}</button>
                          </td>
                    ...
                    return (
                      <tr key={d.id} className={`transition-colors ${isLate?'hover:bg-red-50/20':'hover:bg-slate-50/60'}`}>
                        ...
                        <td className="px-4 py-2.5 max-w-[160px]">
                          <button onClick={()=>setSelectedDeal(d)} className="block truncate text-xs text-slate-600 hover:text-blue-600 hover:underline text-left w-full" title={d.title}>{d.title}</button>
                        </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4"
          ...
          <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl ring-1 ring-slate-200 overflow-hidden">
            ...
            <div className="flex items-start justify-between px-6 py-4 border-b border-slate-100">
              <div className="pr-4">
                <div className="text-sm font-bold text-slate-900 leading-snug">{selectedDeal.title}</div>
                <div className="text-xs text-slate-400 mt-1 font-semibold">{selectedDeal.account_name}</div>
              </div>
              <button onClick={()=>setSelectedDeal(null)}
                className="h-8 w-8 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 shrink-0">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <StagePill stage={selectedDeal.stage}/>
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${
                  selectedDeal.status==='Won' ? 'bg-green-100 text-green-700' :
                  selectedDeal.status==='Lost' ? 'bg-red-100 text-red-600' :
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  <div className="text-lg font-black text-slate-900 tabular-nums">{fmt(selectedDeal.amount)}</div>
                  <div className="text-xs text-slate-400 mt-0.5">Montant MAD</div>
                </div>
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  <div className="text-lg font-black text-slate-900 tabular-nums">{selectedDeal.prob}%</div>
                  <div className="text-xs text-slate-400 mt-0.5">Probabilité</div>
                </div>
                <div className="bg-slate-50 rounded-xl p-3 text-center">
                  <div className={`text-lg font-black tabular-nums ${selectedDeal.missingClosing ? 'text-red-500' : 'text-slate-900'}`}>
                    {selectedDeal.closingYmReal || selectedDeal.closingYm || '—'}
                  </div>
                  <div className="text-xs text-slate-400 mt-0.5">Closing</div>
                </div>
              </div>
              {/* BU lines */}
              {selectedDeal.lines?.length > 0 && (
                <div>
                  <div className="text-xs font-semibold text-slate-500 mb-2">BU / Vendor</div>
                  <div className="space-y-1.5">
                    {[...selectedDeal.lines].sort((a,b)=>b.amount-a.amount).map((l,i)=>(
                      <div key={i} className="flex items-center justify-between text-xs bg-slate-50 rounded-lg px-3 py-2">
                        <span className="font-semibold text-slate-700">{l.sbu} · <span className="text-slate-400 font-normal">{l.vendor||'—'}</span></span>
                        <span className="font-black text-slate-900 tabular-nums">{fmt(l.amount)}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Next step */}
              {selectedDeal.raw?.next_step && (
                <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
                  <div className="text-xs font-semibold text-amber-700 mb-1">Next Step</div>
                  <div className="text-xs text-amber-900">{selectedDeal.raw.next_step}</div>
                </div>
              )}
              {/* Notes */}
              {selectedDeal.raw?.notes && (
                <div className="bg-slate-50 rounded-xl px-4 py-3">
                  <div className="text-xs font-semibold text-slate-500 mb-1">Notes</div>
                  <div className="text-xs text-slate-600 whitespace-pre-wrap">{selectedDeal.raw.notes}</div>
                </div>
              )}
            </div>
            {/* Footer */}
            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end">
              <button onClick={()=>setSelectedDeal(null)}
                className="h-9 px-5 rounded-xl bg-slate-900 text-sm font-semibold text-white hover:bg-slate-800 transition-colors">
                Fermer
              </button>
            </div>
          </div>
