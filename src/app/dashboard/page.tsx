...
import {
  ...
  Users, Building2, MapPin, Calendar,
} from 'lucide-react'
import {
...
export default function Dashboard() {
  ...
  const mixBU = useMemo(()=>{
    ...
    const data=[
      {name:'CIRS',value:mv(cirsA,cirsC),color:C.cirs},
      {name:'CSG', value:mv(csgA, csgC), color:C.csg },
    ]
  ...
  const bySBU = useMemo(()=>{
    ...
    for (const d of openDeals) for (const ln of d.lines) {
      ...
      if (metric==='count'){cur.total++;cur.count++}
    ...
    for (const d of wonDeals) for (const ln of d.lines) {
      const sbu=ln.sbu==='Other'?'Other':String(ln.sbu)
      const cur=map.get(sbu)||{sbu,total:0,forecast:0,won:0,count:0}
      cur.won+=metric==='amount'?ln.amount:1
      map.set(sbu,cur)
    }
  ...
  const funnel = useMemo(()=>{
    ...
    return funnelStages.map(stage=>{
      ...
      return {name:stage,value:metric==='amount'?amount:deals_at_stage.length,count:deals_at_stage.length}
    }).filter(x=>x.value>0)
  },[openDeals,metric])
  ...
  const trend = useMemo(()=>monthsOfYear(year).map((m,i)=>{
    ...
    return {
      ...
      commit: mv2(open.filter(d=>isCommit(d.stage)).reduce((s,d)=>s+d.amount,0),open.filter(d=>isCommit(d.stage)).length),
  ...
  const topClients = useMemo(()=>{
    ...
    for (const d of scopeDeals) {
      ...
      if (metric==='count'){cur.total++;cur.deals++}
      else{for(const ln of d.lines){if(ln.group==='CSG')cur.csg+=ln.amount;else cur.cirs+=ln.amount};cur.total=cur.csg+cur.cirs;cur.deals++}
  ...
  const byRegion = useMemo(()=>{
    ...
    for (const d of scopeDeals) {
      ...
      cur.total+=metric==='amount'?d.amount:1; cur.count++; map.set(region,cur)
    }
  ...
  const bySector = useMemo(()=>{
    ...
    for (const d of scopeDeals) {
      ...
      cur.total+=metric==='amount'?d.amount:1; cur.count++; map.set(sector,cur)
    }
      ...
      <div className="mx-auto max-w-screen-2xl px-4 py-6 space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">{periodLabel} · {inPeriod.length} deals dans la période</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6">
          ...
          <KpiCard label="Taille moy. deal" color="slate" icon={<BarChart2 className="h-5 w-5"/>}
            value={metric==='amount'?fmt(kpis.avgDeal)+' MAD':String(kpis.pipeCount)}
            sub="Open deals"/>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
          ...
          {/* Open/Won/Lost donut */}
          <Panel title="Répartition statuts" sub={`${metric==='amount'?'Montant MAD':'Nb deals'} · Période`}>
            {donut.total<=0?<Empty/>:(
              <>
                <div className="h-48">
                ...
                <div className="mt-3 flex justify-around">
                  {donut.d.map(e=>(
                    <div key={e.name} className="text-center">
                      <div className="text-lg font-black text-slate-900">{metric==='amount'?fmt(e.value):e.value}</div>
                ...
                <div className="mt-3 flex justify-around">
                  {mixBU.data.map(e=>(
                    <div key={e.name} className="text-center">
                      ...
                      <div className="text-xs text-slate-400">{metric==='amount'?fmt(e.value)+' MAD':e.value}</div>
                    </div>
                  ))}
                </div>
              </>
          ...
          <Panel title="Répartition par étape" sub={`Open · ${metric==='amount'?'MAD':'Nb'}`}>
                  ...
                  {byRegion.map(x=>(
                    <div key={x.region} className="flex items-center gap-3">
                      ...
                      <div className="text-xs font-bold text-slate-700 w-20 text-right tabular-nums">
                        {metric==='amount'?fmt(x.total)+' MAD':x.total}
                      </div>
