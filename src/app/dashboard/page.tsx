...
type ScopeMode = 'open_only' | 'won_only' | 'all'
...
export default function Dashboard() {
  ...
  const scopeDeals = useMemo(()=>{
    if (scope==='open_only') return openDeals
    if (scope==='won_only') return wonDeals
    return [...openDeals,...wonDeals,...lostDeals] // all
  },[openDeals,wonDeals,lostDeals,scope])
  ...
  },[openDeals,wonDeals,lostDeals,prevOpenDeals,prevWonDeals,deals,year,ANNUAL_TARGET,invoices])
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      <div className="mx-auto max-w-[1500px] px-4 py-6 space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <SegBtn value={scope} onChange={v=>setScope(v as ScopeMode)}
              items={[{id:'open_only',label:'Open'},{id:'won_only',label:'Won'},{id:'all',label:'Tout'}]}/>
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          ...
          <Panel title="Top Constructeurs / Cartes" sub={`${scope==='open_only'?'Open':scope==='won_only'?'Won':'Tout'}`}>
        ...
        {(byRegion.length>0||bySector.length>0)&&(
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
            <Panel title="Pipeline par Région" sub={`Via comptes associés · ${scope==='open_only'?'Open':scope==='won_only'?'Won':'Tout'}`}>
            ...
            <Panel title="Pipeline par Secteur d'activité" sub={`Via comptes associés · ${scope==='open_only'?'Open':scope==='won_only'?'Won':'Tout'}`}>
        ...
        <div className="flex items-center gap-3 mt-2">
          <div className="h-px flex-1 bg-slate-200" />
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Logistique & Finance</span>
          <div className="h-px flex-1 bg-slate-200" />
        </div>
                    ...
                    return (
                      <tr key={d.id} className={`transition-colors ${isLate?'hover:bg-red-50/20':'hover:bg-slate-50/60'}`}>
                        ...
                        <td className="px-4 py-2.5 max-w-[160px]">
                          <Link href={`/opportunities/${d.id}`} className="block truncate text-xs text-slate-600 hover:text-blue-600 hover:underline" title={d.title}>{d.title}</Link>
                        </td>
